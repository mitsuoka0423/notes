...
## 2.3.4.(オプション)Face APIのレスポンス解説

Face APIの顔検出機能のレスポンスは以下のような形式で返却されます。

```json
[
  {
    "faceId": "c95181ea-5d10-4d71-b1ad-42af1565ca6f", <-- 顔ID
    "recognitionModel": "recognition_01", <-- 利用している機械学習モデル
    "faceRectangle": { <-- 検出した顔の位置
      "width": 179,
      "height": 179,
      "left": 351,
      "top": 159
    },
    "faceLandmarks": { <-- 顔のランドマーク(目や口など)の座標
      "pupilLeft": {
        "x": 401.7,
        "y": 206.8
      },
...
検出可能なランドマークは以下の通りです。
[![Image from Gyazo](https://i.gyazo.com/8304f99152881c1ab93c331fb6509da7.jpg)](https://gyazo.com/8304f99152881c1ab93c331fb6509da7)

https://docs.microsoft.com/ja-jp/azure/cognitive-services/face/concepts/face-detection

左目が閉じているかどうかは、

- `response[0].faceLandmarks.eyeLeftTop`
- `response[0].faceLandmarks.eyeLeftBottom`
...
- LINE BotとFace API組み合わせて、写真の顔検出を行い、起きているか寝ているかを判定するBotを作成しました。
